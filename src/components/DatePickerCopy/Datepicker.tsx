...
const Datepicker = ({
  ...
  useEffect(() => {
    if (
      +format(items[0], 'd') >
        +format(items[items.length - 1 < 0 ? 0 : items.length - 1], 'd') &&
      format(items[0], 'M') ===
        format(items[items.length - 1 < 0 ? 0 : items.length - 1], 'M')
    ) {
      setCustomInterval({
        last: format(items[0], 'd'),
        first: format(items[items.length - 1 < 0 ? 0 : items.length - 1], 'd'),
        monthLeft: format(items[0], 'M'),
        monthRight: format(
          items[items.length - 1 < 0 ? 0 : items.length - 1],
          'M'
        ),
      });
    } else {
      setCustomInterval({
        first: format(items[0], 'd'),
        last: format(items[items.length - 1 < 0 ? 0 : items.length - 1], 'd'),
        monthLeft: format(items[0], 'M'),
        monthRight: format(
          items[items.length - 1 < 0 ? 0 : items.length - 1],
          'M'
  ...
  return (
    <Styled.Root>
      <Styled.Spinner>
        {isLoad && <Spinner width={15} height={15} />}
      </Styled.Spinner>
      <Styled.Slider
        id={id}
        slidesPerView={7}
      ...
      >
        {items.map(item => {
          const isTour = Boolean(
            Object.keys(currentToursGroup).length > 0 &&
              currentToursGroup[format(item, 'M')] &&
              currentToursGroup[format(item, 'M')][format(item, 'd')]
          );
          return (
