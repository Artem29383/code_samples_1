...
const MapContainer = ({
  ...
  useEffect(() => {
    if (!Emitter.eventNames().includes(Event.mapIdle)) {
      Emitter.on(Event.mapIdle, (boundings: Boundings) => {
        ...
        refBoundings.current = boundings;
      ...
      Emitter.on(
        Event.drawEditDone,
        (coords: { [key: string]: { lat: number; lng: number }[] }) => {
          prevLoadingProgressRef.current = -1;
          setLoadingProgress(0);
          ...
          if (bubbledPropertyRef.current) {
            fetchShorthandsInBoundings({
              areas: coords,
              boundings: refBoundings.current,
            });
          } else {
            fetchClusters({ areas: coords, boundings: refBoundings.current });
          }
        }
      );
    }
  ...
  useEffect(() => {
    ...
    Emitter.on(Event.resetMarkersMap, () => {
      clearMarkers();
    });
  }, [
    mapRef,
